...
export function checkObject<T extends { [key: string]: "string" | "number" | "boolean" }>(obj: any, props: T) {
	if (!obj || typeof obj != "object") { return null; }
	let res: { [key in keyof T]: T[key] extends "string" ? string : T[key] extends "number" ? T[key] extends "boolean" ? boolean : number : any } = {} as any;
...
export const Stream: { new(buf: Buffer): Stream, prototype: Stream } = function Stream(this: Stream, data: Buffer, scan = 0) {
	...
	this.tee = function () {
		return new (Stream as any)(data, scan);
	}
	this.eof = function () {
	...
	this.readInt = function (bigendian = false) {
		return this.readUInt(bigendian) | 0;
	}
	this.readStringLine = function () {
		let start = scan;
		while (scan < data.length && data[scan] != 0x0a && data[scan] != 0x0d) { scan++; }
		let res = data.toString("utf8", start, scan);
		if (data[scan] == 0x0d) { scan++; }
		if (data[scan] == 0x0a) { scan++; }
		return res;
	}
...
export class IterableWeakMap<K extends WeakKey, V> {
	...
	static cleanup({ set, ref }: { set: Set<any>, ref: any }) {
		set.delete(ref);
	}
