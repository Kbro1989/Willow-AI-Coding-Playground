...
import { alignedRefOrCopy, ArrayBufferConstructor, buildAttributeBuffer } from "./gltfutil";
import { CacheFileSource } from "../cache";
import { parse } from "../opdecoder";
import type { models } from "../generated/models";
...
export type ModelMeshData = {
	...
	attributes: {
		...
		texuvs?: THREE.BufferAttribute | THREE.InterleavedBufferAttribute,
		//new skeletal animations
		skinids?: THREE.BufferAttribute | THREE.InterleavedBufferAttribute,
		skinweights?: THREE.BufferAttribute | THREE.InterleavedBufferAttribute,
		//old transform based animations
		boneids?: THREE.BufferAttribute | THREE.InterleavedBufferAttribute,
		boneweights?: THREE.BufferAttribute | THREE.InterleavedBufferAttribute
	}
}
...
function interleaveAttributes(input: {
	pos: Float32Array,
	normals?: Int8Array | null,
	color?: Uint8Array | null,
	texuvs?: Float32Array | null,
	skinids?: Uint16Array | null,
	skinweights?: Uint8Array | null,
	boneids?: Uint8Array | null,
	boneweights?: Uint8Array | null,
	vertexCount: number
}) {
	let sources: any = {};
	if (input.pos) sources.pos = { source: input.pos, vecsize: 3, newtype: "f32" };
	if (input.normals) sources.normals = { source: input.normals, vecsize: 3, newtype: "i8" };
	if (input.color) sources.color = { source: input.color, vecsize: 4, newtype: "u8" };
	if (input.texuvs) sources.texuvs = { source: input.texuvs, vecsize: 2, newtype: "f32" };
	if (input.skinids) sources.skinids = { source: input.skinids, vecsize: 4, newtype: "u16" };
	if (input.skinweights) sources.skinweights = { source: input.skinweights, vecsize: 4, newtype: "u8" };
	if (input.boneids) sources.boneids = { source: input.boneids, vecsize: 4, newtype: "u8" };
	if (input.boneweights) sources.boneweights = { source: input.boneweights, vecsize: 4, newtype: "u8" };

	let built = buildAttributeBuffer(sources);

	let ibBox: any = {};
	if (built.bytestride % 4 == 0) ibBox.f32 = new THREE.InterleavedBuffer(new Float32Array(built.buffer.buffer), built.bytestride / 4);
	ibBox.u8 = new THREE.InterleavedBuffer(built.buffer, built.bytestride);
	ibBox.i8 = new THREE.InterleavedBuffer(new Int8Array(built.buffer.buffer), built.bytestride);
	if (built.bytestride % 2 == 0) ibBox.u16 = new THREE.InterleavedBuffer(new Uint16Array(built.buffer.buffer), built.bytestride / 2);

	let attributes: ModelMeshData["attributes"] = {} as any;

	function addAttr(name: keyof ModelMeshData["attributes"], metaName: string, type: string, size: number, normalized = false) {
		if (built.attributes[metaName]) {
			let meta = built.attributes[metaName];
			let ib = ibBox[type];
			if (!ib) throw new Error(`Stride mismatch for ${name} type ${type}`);
			let offset = meta.byteoffset / (type == "f32" ? 4 : type == "u16" ? 2 : 1);
			attributes[name] = new THREE.InterleavedBufferAttribute(ib, size, offset, normalized) as any;
		}
	}

	addAttr("pos", "pos", "f32", 3);
	addAttr("normals", "normals", "i8", 3, true);
	addAttr("color", "color", "u8", 4, true);
	addAttr("texuvs", "texuvs", "f32", 2);
	addAttr("skinids", "skinids", "u16", 4);
	addAttr("skinweights", "skinweights", "u8", 4, true);
	addAttr("boneids", "boneids", "u8", 4);
	addAttr("boneweights", "boneweights", "u8", 4, true);

	return attributes;
}

function getBoneIdData(boneidBuffer: Uint16Array) {
	...
	return { boneids: quadboneids, boneweights: quadboneweights };
}

function getUvData(vertexCount: number, uvBuffer: Uint16Array | Float32Array) {
...
function getNormalsData(normalBuffer: Int8Array | Int16Array) {
...
export function parseOb3Model(modelfile: Buffer, source: CacheFileSource) {
	...
	if (parsed.meshes) {
		for (let mesh of parsed.meshes) {
			...
			let indexlods = indexBuffers.map((q: any) => new THREE.BufferAttribute(q, 1));

			let pos = parsePosData(mesh.positionBuffer!);
			let skinids: Uint16Array | null = null;
			let skinweights: Uint8Array | null = null;
			let color: Uint8Array | null = null;
			let boneids: Uint8Array | null = null;
			let boneweights: Uint8Array | null = null;
			let texuvs: Float32Array | null = null;
			let normals: Int8Array | null = null;
			...
			if (mesh.skin) {
				...
				skinids = skinIdBuffer;
				skinweights = skinWeightBuffer;
			}

			if (mesh.colourBuffer) {
				if (!indexlods[0]) { throw new Error("need index buf in order to read per-face colors"); }
				let indexbuf = indexlods[0].array;
			...
			if (mesh.boneidBuffer) {
				let res = getBoneIdData(mesh.boneidBuffer);
				boneids = res.boneids;
				boneweights = res.boneweights;
			}
			if (mesh.uvBuffer) {
				texuvs = getUvData(mesh.vertexCount, mesh.uvBuffer);
			}
			if (mesh.normalBuffer) {
				normals = getNormalsData(mesh.normalBuffer);
			}

			let attributes = interleaveAttributes({
				pos, normals, color, texuvs, skinids, skinweights, boneids, boneweights, vertexCount: mesh.vertexCount
			});
