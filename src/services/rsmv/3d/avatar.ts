...
export const slotToKitMale: Record<number, number> = { 4: 2, 6: 3, 7: 5, 8: 0, 9: 4, 10: 6, 11: 1 };

//female 7head,9body,10arms,11hands,12legs,13feet,
export const slotToKitFemale: Record<number, number> = { 4: 9, 6: 10, 7: 12, 8: 7, 9: 11, 10: 13 };
		...
		avatar.slots.forEach(slot => {
			...
			if (equip) {
				...
				if (slot.cust?.color?.col2) {
					for (let i in mods.replaceColors) { (mods.replaceColors as any)[i][1] = (slot.cust.color.col2 as any)[i]; }
				}
				if (slot.cust?.color?.col4) {
					mods.replaceColors!.push(...slot.cust.color.col4);
				}
				if (slot.cust?.material) {
					for (let i in mods.replaceMaterials) { (mods.replaceMaterials as any)[i][1] = (slot.cust.material.materials as any)[i]; }
				}
				if (slot.cust?.model) {
					for (let i in slot.cust.model) { (equip.models as any)[i] = (slot.cust.model as any)[i]; }
				}
