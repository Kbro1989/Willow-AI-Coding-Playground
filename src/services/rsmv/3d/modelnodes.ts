...
import { ThreejsSceneCache, mergeModelDatas, ob3ModelToThree, mergeBoneids, constModelsIds } from './modeltothree';
import { ModelModifications, TypedEmitter, CallbackPromise } from '../utils';
import { boundMethod } from 'autobind-decorator';
import { resolveMorphedObject, modifyMesh, MapRect, ParsemapOpts, RSMapChunkData, renderMapSquare, WorldLocation, ThreeJsRenderSection, tiledimensions, parseMapsquare } from './mapsquare';
...
import { mapImageCamera } from "../map/index";
...
export async function playerToModel(cache: ThreejsSceneCache, name: string) {
	...
	if (name.length <= 20) {
		...
		let data = await fetch(url).then((q: any) => q.text());
...
export async function locToModel(cache: ThreejsSceneCache, id: number) {
	...
	if (morphedloc) {
		...
		models = [
			...morphedloc.models?.flatMap((m: any) => m.values).map((q: any) => ({ modelid: q, mods })) ?? [],
			...morphedloc.models_05?.models.flatMap((m: any) => m.values).map((q: any) => ({ modelid: q, mods })) ?? []
		];
	}
...
export async function materialToModel(sceneCache: ThreejsSceneCache, id: number) {
	...
	for (let tex in mat.textures) {
		if ((mat.textures as any)[tex] != 0) {
			await addtex(tex as any, tex, (mat.textures as any)[tex]);
		}
	}
	return castModelInfo({
...
export class RSMapChunkGroup extends TypedEmitter<{ loaded: undefined, changed: undefined }> implements ThreeJsSceneElementSource {
	...
	constructor(cache: ThreejsSceneCache, rect: MapRect, extraopts?: ParsemapOpts) {
		...
		Promise.all(this.chunks.map((q: any) => q.chunkdata)).then((q: any) => {
			this.emit("loaded", undefined);
		});
	}
}
...
export class RSMapChunk extends TypedEmitter<{ loaded: RSMapChunkData, changed: undefined }> implements ThreeJsSceneElementSource {
	...
	async testLocImg(loc: WorldLocation) {
		...
		group.add(...sections.map((q: any) => q.mesh));
		group.traverse((q: any) => q.layers.set(1));
		(this.loaded.chunkroot as any).add(group as any);
	...
	cleanup() {
		this.listeners = {};
		if (this.globalname) {
			delete (globalThis as any)[this.globalname];
			this.globalname = "";
		}
		//only clear vertex memory for now, materials might be reused and are up to the scenecache
		this.chunkdata.then((q: any) => q.chunkroot.traverse((obj: any) => {
