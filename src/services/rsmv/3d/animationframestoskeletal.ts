...
function buildFramebaseSkeleton(framebase: framemaps) {
    ...
    let maxbone = Math.max(...framebase.data.flatMap((q: any) => q.data));
    ...
    for (let [index, base] of framebase.data.entries()) {
        if (base.type == 0) {
            if (base.data.length == 0) {
                ...
                pivot = findSharedPivot(base.data.map((q: any) => bones[q + 1]));
    ...
    console.log(framebase.data.map((q: any) => [q.type, "", ...q.data.map((q: any) => q + 1)]));
