...
import type { skeletalanim } from "../generated/skeletalanim";
import type { framemaps } from "../generated/framemaps";
import { ThreejsSceneCache } from "./modeltothree";
import type { sequences } from "../generated/sequences";
...
function debugBezierCurve(x0: number, x1: number, x2: number, x3: number, x: number) {
	...
	let best = 100;
	let bestt = 0;
	let pxtoy = (v: number) => ((1 - (v - x0) / (x3 - x0)) * 0.5 + 0.25) * cnv.height;
	let pxtox = (t: number) => (t * 0.5 + 0.25) * cnv.width;
...
function debugkeyframes(data: number[], times: number[], axis: number) {
	...
	ctx.strokeStyle = "red";
	ctx.lineWidth = 2;
	let pxtoy = (v: number) => ((1 - (v - min) / (max - min)) * 0.8 + 0.1) * cnv.height;
	let pxtox = (t: number) => (t / duration) * cnv.width;
