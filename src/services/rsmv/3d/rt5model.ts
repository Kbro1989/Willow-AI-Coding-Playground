...
import type { oldmodels } from "../generated/oldmodels";
...
export function parseRT5Model(modelfile: Buffer, source: CacheFileSource) {
    ...
    if (modeldata.texflags) {
        ...
        for (let i = 0; i < modeldata.texflags.length; i++) {
            ...
            if ((globalThis as any).testmat >= 0 && (globalThis as any).testmat == i) {
    ...
    for (let i = 0; i < modeldata.facecount; i++) {
        ...
        if (matargument) {
            ...
            if ((globalThis as any).testmat >= 0 && (globalThis as any).testmat != mapid - 1) {
