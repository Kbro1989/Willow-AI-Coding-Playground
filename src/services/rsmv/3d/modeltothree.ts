...
import { ModelData, parseOb3Model } from './rt7model';
import { parseRT5Model } from "./rt5model";
...
import type { mapscenes } from "../generated/mapscenes";
...
import type { maplabels } from "../generated/maplabels";
...
export function augmentThreeJsFloorMaterial(mat: THREE.Material, isminimap: boolean) {
	...
	mat.onBeforeCompile = (shader, renderer) => {
		...
		const fragmentHeader = `#ifdef USE_MAP\n`
			+ `varying vec2 v_ra_floortex_0;\n`
			+ `varying vec2 v_ra_floortex_1;\n`
			+ `varying vec2 v_ra_floortex_2;\n`
			+ `varying vec3 v_ra_floortex_weights;\n`
		...
		let mapFragment = `#include <color_fragment>\n#ifdef USE_MAP\nvec4 texelColor = \n`;
		if (isminimap) {
			mapFragment += `   v_ra_floortex_weights.r * mix(texture2D( map, v_ra_floortex_0 ), diffuseColor * 0.5, v_ra_floortex_usescolor.r)\n`
			...
			mapFragment += `   texture2D( map, v_ra_floortex_0 ) * v_ra_floortex_weights.r * mix(vec4(1.0), diffuseColor, v_ra_floortex_usescolor.r)\n`
		...
		mapFragment += `texelColor = mix( diffuseColor,texelColor,dot(vec3(1.0),v_ra_floortex_weights));\n#endif\ndiffuseColor = texelColor;\n`;
...
async function convertMaterialToThree(source: ThreejsSceneCache, material: MaterialData, hasVertexAlpha: boolean, minimapVariant: boolean) {
	...
	if (minimapVariant) {
		...
		// Use standard material for legacy if needed, or we could also use RS material
	} else {
		// Main integration: use the extracted RuneScape shaders
	...
	if (needsvertexcolors) {
		if (parsedmat.matmeta.baseColorFraction != 0) {
			...
			for (let i = 0; i < vertcount; i++) {
				...
				let oldr = (hasoldcol ? (oldbuf as any)[jj + 0] : 255);
				let oldg = (hasoldcol ? (oldbuf as any)[jj + 1] : 255);
				let oldb = (hasoldcol ? (oldbuf as any)[jj + 2] : 255);
				(newbuf as any)[ii + 0] = clamp(oldr * oldfrac + newrcomp);
				(newbuf as any)[ii + 1] = clamp(oldg * oldfrac + newgcomp);
				(newbuf as any)[ii + 2] = clamp(oldb * oldfrac + newbcomp);
				if (hasVertexAlpha) {
					(newbuf as any)[ii + 3] = (hasoldcol ? (oldbuf as any)[jj + 3] : 255);
				}
			}
		...
		if (i > start + 1) {
			...
			for (let j = start; j < i; j++) {
				...
				if (j != start && mesh1.attributes.boneids && mesh1.attributes.boneweights && mesh2.attributes.boneids && mesh2.attributes.boneweights) {
					...
					// Mesh1 source, Mesh2 destination
					// copyVal(mesh2.attributes.boneids, i2, mesh1.attributes.boneids, i1);
					// copyVal(mesh2.attributes.boneweights, i2, mesh1.attributes.boneweights, i1);

					const copyVal = (dest: THREE.BufferAttribute | THREE.InterleavedBufferAttribute, idest: number, src: THREE.BufferAttribute | THREE.InterleavedBufferAttribute, isrc: number) => {
						if ((dest as any).isInterleavedBufferAttribute) {
							if (!(src as any).isInterleavedBufferAttribute) throw new Error("Mismatch attribute type in merge");
							// Manual copy for interleaved
							const d = dest as THREE.InterleavedBufferAttribute;
							const s = src as THREE.InterleavedBufferAttribute;
							for (let k = 0; k < d.data.stride; k++) {
								// This is tricky as we need to match the specific offset. 
								// Actually, simpler: use set/get component.
								// But getting X,Y,Z,W allows up to 4 components.
								for (let c = 0; c < dest.itemSize; c++) {
									// This is slow but safe
									dest.setComponent(idest, c, src.getComponent(isrc, c));
								}
							}
						} else {
							(dest as THREE.BufferAttribute).copyAt(idest, src as THREE.BufferAttribute, isrc);
						}
					}
...
export { ob3ModelToThree as modelToThree };
