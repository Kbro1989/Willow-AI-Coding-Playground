...
export class ClientScriptSubtypeSolver {
    ...
    entangle(key: number, other: number | undefined) {
        ...
        if (Array.isArray((globalThis as any).testkey) && key == (globalThis as any).testkey[0] && other == (globalThis as any).testkey[1]) {
            // Removed debugger
        }
        if (Array.isArray((globalThis as any).testkey) && key == (globalThis as any).testkey[1] && other == (globalThis as any).testkey[0]) {
            // Removed debugger
        }
        if (typeof (globalThis as any).testboth == "number" && (key == (globalThis as any).testboth || other == (globalThis as any).testboth)) {
            // Removed debugger
        }
                ...
                if (links) {
                    ...
                    for (let link of links) {
                        ...
                        if (typeof prevknown == "undefined") {
                            ...
                            (globalThis as any).testkey = [key, link];
...
class CombinedExactStack {
    ...
    pushopcode(node: RawOpcodeNode, scriptid: number) {
        if (Array.isArray((globalThis as any).test) && (globalThis as any).test[0] == scriptid && (globalThis as any).test[1] == node.originalindex) {
            // Removed debugger
        }
