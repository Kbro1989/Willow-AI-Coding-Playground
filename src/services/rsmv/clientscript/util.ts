...
export function getEnumInt(enumjson: enums, key: number) {
    ...
    return (enumjson.intArrayValue1 ?? enumjson.intArrayValue2?.values)?.find((q: any) => q[0] == key)?.[1] ?? enumjson.intValue ?? 0;
}

export function getEnumString(enumjson: enums, key: number) {
    return (enumjson.stringArrayValue1 ?? enumjson.stringArrayValue2?.values)?.find((q: any) => q[0] == key)?.[1] ?? enumjson.stringValue ?? "";
}
