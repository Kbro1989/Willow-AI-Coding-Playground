...
import type { dbtables } from "../generated/dbtables";
...
const detectableImmediates: ImmediateType[] = ["byte", "int", "tribyte", "switch"];
...
export class ClientscriptObfuscation {
    ...
    static async create(source: CacheFileSource, nocached = false) {
        ...
        (globalThis as any).deob = res;//TODO remove
            ...
            while (row.length != 0) {
                ...
                if (matched) {
                    ...
                    if (op.possibleTypes.size == 1 && op.type == "unknown") {
                        op.type = op.possibleTypes.values().next().value as ImmediateType;
                    }
...
function callibrateOperants(calli: ClientscriptObfuscation, candidates: Map<number, ScriptCandidate>) {
    ...
    (globalThis as any).allsections = allsections;//TODO remove
...
//TODO remove/hide
(globalThis as any).getop = (opid: string) => {
    let id = -1;
    ...
    let calli: ClientscriptObfuscation = (globalThis as any).deob;
