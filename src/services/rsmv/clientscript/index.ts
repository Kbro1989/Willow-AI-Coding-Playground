...
export async function renderClientScript(source: CacheFileSource, buf: Buffer, fileid: number, relativeComps = false, notypes = false, int32casts = false) {
    ...
    if (buf.length > 0) {
        script = parse.clientscript.read(buf, source);
    } else {
        // Fallback for when we only have the ID or JSON already parsed (used by UI)
        let file = await source.getFileById(cacheMajors.clientscript, fileid);
        script = parse.clientscript.read(file, source);
    }
...
export async function prepareClientScript(source: CacheFileSource) {
    if (!source.decodeArgs.clientScriptDeob) {
        ...
        (globalThis as any).deob = deob;//TODO remove
    }
