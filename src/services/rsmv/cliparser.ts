...
function cacheSourceFromString(str: string) {
	...
	return async (opts: CacheOpts) => {
		switch (mode) {
			case "live":
				return import("./cache/downloader").then(m => new m.CacheDownloader());
			case "auto":
				let fs = new CLIScriptFS(arg);
				return selectFsCache(fs, opts);
			case "nxt":
			case "cache":
				return import("./cache/sqlite").then(m => new m.GameCacheLoader(arg, opts?.writable ?? false));
			case "cache-write":
				return import("./cache/sqlite").then(m => new m.GameCacheLoader(arg, true));
			case "openrs":
			case "openrs2":
			...
			case "extracted":
				return import("./cache/rawfiles").then(m => new m.RawFileLoader(arg, 0));
			case "global":
				let fn = (globalThis as any)[arg];
...
const FileRangeType: cmdts.Type<string, FileRange[]> = {
...
export var filerange = literal({
	files: cmdts.option({ long: "ids", short: "i", type: FileRangeType, defaultValue: () => [{ start: [0, 0, 0], end: [Infinity, Infinity, Infinity] }] as FileRange[] })
});
