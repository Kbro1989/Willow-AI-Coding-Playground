...
export class CachingFileSource extends CacheFileSource {
    ...
    private cacheFetchCounter: number = 0;
    private cacheAddCounter: number = 0;
    maxcachesize: number = 200e6;
    ...
    fetchCachedObject<T>(map: Map<number, CachedObject<T>>, id: number, create: () => Promise<T>, getSize: (obj: T) => number): Promise<T> {
        ...
        if (!bucket || (globalThis as any).ignoreCache) {
            let data: Promise<T> = create();
            bucket = {
                promise: data,
                data: null,
                owner: map,
