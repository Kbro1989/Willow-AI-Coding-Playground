...
(globalThis as any).ignoreCache = false;
...
export function unpackBufferArchive(buffer: Buffer, subids: number[], namehashes: number[] | null): SubFile[] {
	if (subids.length == 1) {
		let r: SubFile[] = [{
			buffer: buffer,
			offset: 0,
...
export function rootIndexBufferToObject(metaindex: Buffer, source: CacheFileSource): CacheIndex[] {
...
export const mappedFileIds: Record<number, number> = {
...
export const oldConfigMaps: Record<number, number> = {
...
export function archiveToFileId(major: number, minor: number, subfile: number): number {
	let archsize = mappedFileIds[major] ?? 1;
...
export abstract class CacheFileSource {
	...
	async getIndexEntryById(major: number, minor: number): Promise<CacheIndex> {
	...
	async getArchiveById(major: number, minor: number): Promise<SubFile[]> {
		let index = await this.getIndexEntryById(major, minor);
		return this.getFileArchive(index);
	}

	async getFileById(major: number, fileid: number): Promise<Buffer> {
	...
	async findFileByName(major: number, name: string): Promise<CacheIndex | undefined> {
		let hash = cacheFilenameHash(name, this.getBuildNr() <= lastLegacyBuildnr);
		let indexfile = await this.getCacheIndex(major);
		return indexfile.find(q => q && q.name == hash);
	}
	async findSubfileByName(major: number, minor: number, name: string): Promise<SubFile | undefined> {
		let hash = cacheFilenameHash(name, this.getBuildNr() <= lastLegacyBuildnr);
	...
	//for testing only
	async bruteForceFindAnyNamedFile(name: string): Promise<SubFile[] | null> {
