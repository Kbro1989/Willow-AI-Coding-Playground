...
export async function downloadServerConfig() {
	let body: string = await fetch("http://world3.runescape.com/jav_config.ws?binaryType=2").then((r: any) => r.text());
	...
	for (var i = 0; i < chunks.length; ++i) {
		...
		if (line.length == 2) {
			...
			config[line[0]] = (config[line[0]] || {}) as any;
			(config[line[0]] as any)[line[1]] = line[2];
		}
	}
