...
export class FileSourceFsCache {
    ...
    static tryCreate() {
        if (typeof (globalThis as any).__non_webpack_require__ == "undefined") { return null; }
        try {
            (globalThis as any).__non_webpack_require__("sqlite3")
        } catch {
            return null;
        }
    ...
    constructor(filename: string) {
        ...
        this.ready = (async () => {
            if (typeof (globalThis as any).__non_webpack_require__ !== "undefined") {
                const fs = (globalThis as any).__non_webpack_require__("fs/promises");
