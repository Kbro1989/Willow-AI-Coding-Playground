...
import type { FileSourceFsCache } from "./fscache";
...
export function loadOpenrsCachelist() {
	cachelist ??= fetch(`${endpoint}/caches.json`).then((q: any) => q.json());
	return cachelist;
}
...
export class Openrs2CacheSource extends cache.DirectCacheFileSource {
	...
	constructor(meta: Openrs2CacheMeta) {
		...
		if (typeof (globalThis as any).__non_webpack_require__ !== "undefined") {
			import("./fscache").then(m => this.fscache = m.FileSourceFsCache.tryCreate());
		}
	}
	...
	async getCacheIndex(major: number) {
		if (this.buildnr <= 700 && !this.xteaKeysLoaded && major == cacheMajors.mapsquares) {
			this.xteakeysPromise ??= (async () => {
				...
				let keys: Openrs2XteaKey[] = await fetch(`${endpoint}/caches/runescape/${this.meta.id}/keys.json`).then((q: any) => q.json());
				for (let key of keys) {
