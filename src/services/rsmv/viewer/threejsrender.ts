...
//console hooks
(globalThis as any).logclicks = false;
(globalThis as any).speed = 100;
...
export type RenderCameraMode = "standard" | "vr360" | "item";
...
export class ThreeJsRenderer extends TypedEmitter<ThreeJsRendererEvents> {
	...
	constructor(canvas: HTMLCanvasElement, params?: THREE.WebGLRendererParameters) {
		...
		//three typings are updated in newer r182
		...
		this.standardLights.add(ambientlight as any);
		this.standardLights.add(dirLight as any);
		this.standardLights.add(hemilight as any);
	...
	sceneElementsChanged() {
		...
		for (let source of this.sceneElements) {
			...
			for (let el of elgroup) {
				...
				if (el.modelnode) {
					nodeDeleteList.delete(el.modelnode as any);
					if (el.modelnode.parent != this.modelnode) {
						this.modelnode.add(el.modelnode as any);
					}
				}
			}
		}
		...
		if (sky?.skybox) {
			...
			obj.add(sky.skybox as any);
			scene.clear();
			scene.add(obj as any, camera as any, new THREE.AmbientLight(0xffffff) as any);
			scene.background = (sky.fogColor ? fogobj.color.clone() : null);
			this.skybox = { scene, camera: camera as THREE.Camera };
		} else {
			this.skybox = null;
		}
	...
	render(cam?: THREE.Camera) {
		...
		delta *= ((globalThis as any).speed ?? 100) / 100;//TODO remove
	...
	async takeScenePicture(width = this.canvas.width, height = this.canvas.height) {
		...
		if (width != this.canvas.width || height != this.canvas.height) {
			...
			rendertarget = new THREE.WebGLRenderTarget(width, height, {
				...
				colorSpace: (this.camMode != "vr360" ? this.renderer.outputColorSpace : THREE.LinearSRGBColorSpace) as THREE.ColorSpace,
	...
	async click(cnvx: number, cnvy: number) {
		...
		let currentcam = this.camMode == "standard" ? this.getStandardCamera() : this.camMode == "item" ? this.getItemCamera() : null;
		...
		for (let isct of intersects) {
			let obj: THREE.Object3D | null = isct.object as unknown as THREE.Object3D;
			if (!obj.visible) { continue; }
			let meshdata = obj.userData as ModelExtras;

			if (firstloggable) {
				(globalThis as any).model = isct.object;
				firstloggable = false;
				if ((globalThis as any).logclicks) {
					if (isct.object instanceof Mesh && isct.object.geometry instanceof BufferGeometry) {
						...
						for (let [id, attr] of Object.entries((isct.object as THREE.Mesh).geometry.attributes as any) as [string, THREE.BufferAttribute][]) {
							let vals: number[][] = [];
			...
			let startindex = 0;
			let clickindex = isct.faceIndex ?? 0;
	...
	makeUIRenderer() {
		...
		scene.add(new THREE.AmbientLight(0xffffff, 0.7) as any);
		let hemilight = new THREE.HemisphereLight(0xffffff, 0x888844);
		var dirLight = new THREE.DirectionalLight(0xffffff);
		dirLight.position.set(75, 300, -75);
		...
		scene.add(dirLight as any);
		scene.add(hemilight as any);
		scene.add(modelnode);
		...
		let setmodel = (model: ThreeJsSceneElement | null, centery: number) => {
			if (currentnode?.modelnode) {
				modelnode.remove(currentnode.modelnode as any);
				currentnode = null;
			}
			if (model?.modelnode) {
				modelnode.add(model.modelnode as any);
				currentnode = model;
			}
	...
	dispose() {
		...
		disposeThreeTree(this.scene as any);
	}
}

export function disposeThreeTree(node: THREE.Object3D | null) {
	if (!node) { return; }
	const meshNode = node as any; // Cast specifically to access material if needed, or generic Object3D processing
