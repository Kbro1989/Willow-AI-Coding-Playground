...
export function RsUIViewer(p: { data: string }) {
	...
	let render: ThreeJsRenderer = (globalThis as any).render;//TODO
...
function CallbackDebugger(p: { ctx: UiRenderContext, comp: RsInterfaceComponent, source: CacheFileSource }) {
	return (
		<div>
			{Object.entries(p.comp.data.scripts).filter((q: [string, any]) => q[1] && q[1].length != 0).map(([key, v]: [string, any]) => {
				...
				return (
					<div key={key} className="rs-componentcallback" onClick={e => p.ctx.runClientScriptCallback(p.comp.compid, v)}>
						{key} {callbackid}({(v.slice(1) as any[]).map((q: any) => typeof q == "string" ? `"${q}"` : q).join(",")})
					</div>
				)
			})}
		</div>
