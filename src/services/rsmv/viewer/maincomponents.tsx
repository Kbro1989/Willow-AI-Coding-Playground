...
export class CacheSelector extends React.Component<{ onOpen: (c: SavedCacheSource) => void, noReopen?: boolean }, { lastFolderOpen: FileSystemDirectoryHandle | null }> {
	...
	async clickReopen() {
		...
		if (await (this.state.lastFolderOpen as any).requestPermission() == "granted") {
		...
		if (e.dataTransfer) {
			...
			await Promise.all(items.map(async item => {
				if ((item as any).getAsFileSystemHandle) {
					let filehandle = (await (item as any).getAsFileSystemHandle())!;
					if (filehandle.kind == "file") {
						...
						for await (let handle of (dir as any).values()) {
							if (handle.kind == "file") {
...
export async function openSavedCache(source: SavedCacheSource, remember: boolean) {
	...
	if (source.type == "sqliteblobs" || source.type == "autohandle") {
		if (source.type == "autohandle") {
			let perm = await (source.handle as any).queryPermission({ mode: "read" });
			if (perm == "granted") {
...
export function FileDisplay(p: { file: UIOpenedFile }) {
	...
	if (ext == "hexerr.json") {
		...
		let fileid = +(p.file.name.match(/(\d+)/)?.[1] ?? 0);
		el = <ClientScriptViewer data={fileText()} fileid={fileid} />
	} else if (ext == "html") {
