...
export class WebFsScriptFS implements ScriptFS {
	...
	async readDir(dirname: string) {
		...
		for await (let [name, v] of (dir as any).entries()) {
	...
	async writeFile(name: string, data: Buffer | string) {
		...
		let str = await (file as any).createWritable({ keepExistingData: false });
...
export function UIScriptFiles(p: { fs?: UIScriptFS | null, ctx: UIContext }) {
	...
	useEffect(() => {
		(globalThis as any).scriptfs = p.fs;
		return () => {
			(globalThis as any).scriptfs = null;
		}
	})
	...
	if (!files) {
		...
		let clicksave = async () => {
			...
			if (false) {
				...
				// @ts-ignore
				let dir = await window.showDirectoryPicker({ mode: "readwrite", startIn: "downloads" });
