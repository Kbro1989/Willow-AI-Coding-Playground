...
type LookupMode = "model" | "item" | "npc" | "object" | "material" | "avatar" | "spotanim" | "scenario" | "scripts";
...
function propOrDefault<T extends { [key: string]: number | string | boolean }>(v: unknown, defaults: T) {
	...
	if (typeof v == "object" && v) {
		for (let prop in defaults) {
			if (typeof (v as any)[prop] == typeof defaults[prop]) {
				r[prop] = (v as any)[prop];
			}
		}
	}
	return r;
...
export function ModelBrowser(p: { ctx: UIContext }) {
	...
	let [state, setMode] = React.useReducer((prev: state | null, v: LookupMode) => {
...
type ScenarioComponentType = "simple" | "custom";
...
type ModelInitTypes = SimpleModelInitTypes | "custom";
...
export class SceneScenario extends React.Component<LookupModeProps, ScenarioInterfaceState> {
	...
	loadFromJson(str: unknown, isinit = false) {
		...
		this.idcounter = (keys.length == 0 ? 0 : Math.max.apply(null, keys as any) + 1);
		this.hadctx = false;
		if (isinit) {
...
function ScenePlayer(p: LookupModeProps) {
	...
	const colorDropdown = (id: keyof avataroverrides, v: number, opts: Record<number, number>) => {
		return (
			<LabeledInput label={id as string}>
				<select value={v} onChange={e => changeColor(id, +e.currentTarget.value)} style={{ backgroundColor: hsl2hex(opts[v]) }}>
					{Object.entries(opts).map(([i, v_val]) => <option key={i} value={i} style={{ backgroundColor: hsl2hex(v_val) }}>{i}</option>)}
				</select>
			</LabeledInput>
		)
	}
		...
		<React.Fragment>
			...
			<div className="mv-sidebar-scroll">
				...
				<div style={{ userSelect: "text" }}>
					{p.ctx && data && (data.info.avatar?.slots as any[]).map((q: any, i: number) => {
						return (
		...
		<div>
			...
			{slot && cust?.color?.col4 && (
				<div style={{ display: "grid", gridTemplateColumns: `repeat(4, minmax(0, 1fr)) min - content`, gridTemplateRows: "auto auto", gridAutoFlow: "column" }}>
					{cust.color.col4.map((v_item: [number, number], i: number) => (
						<React.Fragment key={i}>
							<InputCommitted type="number" value={v_item[0]} onChange={e => editcust(c => c.color!.col4![i][0] = +e.currentTarget.value)} />
							<InputCommitted type="color" value={hsl2hex(v_item[1])} onChange={e => editcust(c => c.color!.col4![i][1] = hex2hsl(e.currentTarget.value))} />
						</React.Fragment>
					))}
...
export function RendererControls(p: { ctx: UIContext }) {
	...
	let [camMode, setcammode] = React.useState<RenderCameraMode>("standard");
...
async function materialIshToModel(sceneCache: ThreejsSceneCache, reqid: MaterialIshId) {
	...
	for (let tex in mat.textures) {
		if ((mat.textures as any)[tex] != 0) {
			await addtex(tex as any, tex, (mat.textures as any)[tex], mat.stripDiffuseAlpha && tex == "diffuse");
		}
	}
	json = mat;
...
function CacheDiffScript(p: UiScriptProps) {
	...
	let clickOpen = () => {
		let frame = showModal({ title: "Select a cache" }, (
			<CacheSelector onOpen={v => { openCache(v); frame.close(); }} />
		));
	}
