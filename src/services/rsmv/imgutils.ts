...
export async function pixelsToImageFile(imgdata: ImageData, format: "png" | "webp", quality: number) {
	if (typeof HTMLCanvasElement != "undefined") {
		...
		// @ts-ignore
		const req = (globalThis as any).__non_webpack_require__;
...
(globalThis as any).dumptex = dumpTexture;
...
export function drawTexture(ctx: CanvasRenderingContext2D, img: ImageData | Texture | CanvasImage) {
	...
	if ("data" in img) {
		...
		cnv.width = (img as any).source.data.width;
		cnv.height = (img as any).source.data.height;
		ctx.drawImage((img as any).source.data, 0, 0);
	} else {
		cnv.width = img.width;
		cnv.height = img.height
		ctx.drawImage(img, 0, 0);
