...
export async function testDecode(output: ScriptOutput, outdir: ScriptFS, source: CacheFileSource, mode: JsonBasedFile, ranges: FileRange[], opts: ReturnType<typeof defaultTestDecodeOpts>) {
	...
	fileiter = async function* () {
		...
		for (let file of files) {
			...
			if ((globalThis as any).testDecodeFilter && !(globalThis as any).testDecodeFilter(entry)) {
				continue;
			}
			if (orderBySize) {
				allfiles.push(entry);
	...
	function testFile(file: DecodeEntry) {
		...
		if (output.state == "running") {
			if (!(globalThis as any).testDecodeOutputFilter || (globalThis as any).testDecodeOutputFilter(res.state, res.debugdata.rootstate)) {
				if (res.success) {
					nsuccess++;
				} else {
