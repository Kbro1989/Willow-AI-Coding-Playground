...
export async function indexOverview(output: ScriptOutput, outdir: ScriptFS, source: CacheFileSource) {
	...
	for (let indexfile of rootindex) {
		...
		let minorcount = index.reduce((a: number, v: any) => a + 1, 0);
		let subfilecount = index.reduce((a: number, v: any) => a + v.subindexcount, 0);
		let maxsubfiles = index.reduce((a: number, v: any) => Math.max(a, v.subindexcount), 0);
		let minsubfiles = index.reduce((a: number, v: any) => Math.min(a, v.subindexcount), Infinity);
		let totalsize = 0;
	...
	outdir.writeFile("indexoverview.json", JSON.stringify({ majors, configs }, null, '\t'));
}
