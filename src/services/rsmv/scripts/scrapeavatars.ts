...
export async function scrapePlayerAvatars(output: ScriptOutput, outdir: ScriptFS, source: CacheFileSource | null, skip: number, max: number, parsed: boolean) {
	...
	for await (let file of fetchPlayerAvatars(skip, max)) {
		...
		if (parsed) {
			let data = await avatarToModel(scene!.engine, Buffer.from(file.buf), false);
			await outdir.writeFile(`playerdata_${file.name}.json`, prettyJson(data.info.avatar));
		} else {
			outdir.writeFile(`playerdata_${file.name}.bin`, Buffer.from(file.buf));
		}
	}
}

async function* fetchPlayerAvatars(skip: number, max: number): AsyncGenerator<{ name: string, buf: Uint8Array }, void, unknown> {
	let count = 0;
	const pagesize = 25;
