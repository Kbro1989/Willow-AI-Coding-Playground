...
const ShaderGraph: React.FC<ShaderGraphProps> = ({ onCompile, onApplyToObjects }) => {
  ...
  const compileToGLSL = () => {
    ...
    const variableMap = new Map<string, string>(); // nodeID -> variableName
    let varCounter = 0;
    ...
    const resolveNode = (nodeId: string): string => {
      ...
      switch (node.type) {
        ...
        case 'normal':
          ...
          // Add varying if not present (simplified for now)
  ...
  return (
    <div className="h-full flex flex-col bg-[#050a15] overflow-hidden">
      ...
      >
          ...
          >
            ...
            <div className="p-3 space-y-2">
              ...
              {node.type === 'color' && (
                ...
                <div className="text-[10px] font-mono text-cyan-500 text-center animate-pulse py-1">T_DELTA</div>
              )}
        ...
        {/* Port Connection Line (Current Drag) */}
        {activePort && (
          <svg className="absolute inset-0 pointer-events-none">
            {/* Logic to draw a temporary line from activePort to mouse cursor would go here */}
          </svg>
        )}
